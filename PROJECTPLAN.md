...
> **Status:** Active Development — Phase 1 (RAG Backend) | **Last Updated:** 2026-07-27
...
| Completed Phases | Phase 0 — Scaffolding ✅ |
| Active Phase | Phase 1 |
| Next Milestone | Phase 1 completion (Prompt Builder → LLM → Citations → Query API) |
| Blockers | None |
...
|------|----------|--------|
| Local document RAG (PDF, DOCX, TXT, MD) | P0 | 🚧 In Progress (~65%) |
...
Each chunk stores: Text, Page Number (from LangChain metadata), File Name, Chunk ID (auto-generated by Chroma)
...
### 4.4 Retrieval Service ✅ (Implemented — Basic)

ChromaDB similarity search (cosine distance) is available. Future work includes metadata filtering, MMR, and reranking.

### 4.5 Prompt Builder ❌ (Not Yet Implemented)

Will receive `Question + Retrieved Chunks` and produce a structured prompt:
`System Prompt + Context + Instructions + User Question`

### 4.6 LLM Generator ❌ (Not Yet Implemented)

Will have single responsibility: `Prompt → Gemini/OpenAI → Answer`

### 4.7 Citation Service ❌ (Not Yet Implemented)

Every answer will include metadata-backed citations (source, page number).

### 4.8–4.13 Agents, Router, Memory, Reports ❌ (Future)

---

## 5. Data Flows

### RAG (Document Query) — Implemented up to Vector Store

```
User uploads PDF → Document Loader → Text Splitter → Embedding Model → ChromaDB (✅)
                                                                   
User Question → Retriever → Relevant Chunks → Prompt Builder → LLM → Answer + Citations
                (✅)            (✅)            (❌)          (❌)        (❌)
```
...
| 1.7 | Prompt builder | Construct structured prompts from query + chunks | P0 | ⏳ Planned | 1.6 |
| 1.8 | LLM integration | Generate answers via Gemini/OpenAI | P0 | ⏳ Planned | 1.7 |
| 1.9 | Source citations | Attach metadata citations to answers | P0 | ⏳ Planned | 1.8 |
...
- [ ] User can ask a question and get back an answer with source citations
